...
(defn buscar [clave iterable] (
  cond
    (= (.indexOf iterable clave) -1) "ERROR :unbound-variable: f"
    :else (.indexOf iterable clave)
...
; true
(defn error-or-warning? [value]
  (or 
    (= value (symbol ";ERROR:"))
    (= value (symbol ";WARNING:")))
)
(defn error? [iterable]
  (not (every? false? (map error-or-warning? iterable)))
)
...
(defn igual? [value1 value2] (
  cond
    ...
    :else (
      = 
        (.toUpperCase (str value2))
        (.toUpperCase (str value1))
    )
)) 
...
(defn error-message-NaL [the-list]
  (str 
    "(;ERROR: +: Wrong type in arg "
    (nth the-list (.indexOf (map list? the-list) false))
  )
)

(defn fnc-append [iterable] 
  (cond
    (every? list? iterable) (reduce concat iterable)
    :else (error-message-NaL iterable)
  )
)
...
; FUNCION AUXILIAR: error-message-NaN

(defn index-of-first-NaN [the-list] 
  (.indexOf (map number? the-list) false)
)

(defn error-message-NaN [the-list index]
  (str 
    "(;ERROR: +: Wrong type in arg"
    (+ index 1)
    " "
...
(defn fnc-restar [numbers] (
  cond
    (empty? numbers) "(;ERROR: -: Wrong number of args given)"
    (every? number? numbers) (reduce - numbers)
    :else (error-message-NaN numbers (index-of-first-NaN numbers))
))
...
(defn evaluar-escalar [escalar lista] 
  (concat (list escalar) (list lista))
)
